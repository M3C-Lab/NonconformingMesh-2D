...
n_2d = [n1(1); n1(2)];
loc_hh = 1 / sqrt(dot(n_2d, inv(local_basis.FFF) * n_2d));
oppo_hh = 1 / sqrt(dot(n_2d, inv(oppo_basis.FFF) * n_2d));
...
for A = 1 : DataArrays.nLocBas
    ...
    for B = 1 : DataArrays.nLocBas
        ...
        K_oppo_oppo(A, B) = K_oppo_oppo(A, B) ...
            - (NA2 * (NB2_x * n1(1) + NB2_y * n1(2)) + (NA2_x * n1(1) + NA2_y * n1(2)) * NB2) ...
            + penalty * NA2 * NB2;

        K_loc_oppo(A, B) = K_loc_oppo(A, B) ...
            + (NA1 * (NB2_x * n1(1) + NB2_y * n1(2)) + (NA1_x * n1(1) + NA1_y * n1(2)) * NB2) ...
            - penalty * NA1 * NB2;

        K_oppo_loc(A, B) = K_oppo_loc(A, B) ...
            + (NA2 * (NB1_x * n1(1) + NB1_y * n1(2)) + (NA2_x * n1(1) + NA2_y * n1(2)) * NB1) ...
            - penalty * NA2 * NB1;
